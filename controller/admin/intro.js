...
exports.updateIntro = async (req, res, next) => {
    try {
        ...
        const selectSql_cn = `SELECT * FROM displaytext_cn WHERE name='intro'`;
        const selectSql_en = `SELECT * FROM displaytext_en WHERE name='intro'`;
        ...
        if (existingIntro_cn.length === 0) {
            ...
            await queryData(insertSql_cn, { name: 'intro', text: intro_cn });
        } else {
            // 如果 "intro" 行存在，更新该行
            const updateSql_cn = `UPDATE displaytext_cn SET text = ? WHERE name='intro'`;
            await queryData(updateSql_cn, intro_cn);
        }

        if (existingIntro_en.length === 0) {
            const insertSql_en = `INSERT INTO displaytext_en SET ?`;
            await queryData(insertSql_en, { name: 'intro', text: intro_en });
        } else {
            const updateSql_en = `UPDATE displaytext_en SET text = ? WHERE name='intro'`;
...
exports.addIntro = async (req, res, next) => {
    try {
        const sql = `SELECT * FROM displaytext_cn WHERE name = 'intro'`
...
//添加图片
exports.addIntroImages = async (req, res, next) => {
    try {
